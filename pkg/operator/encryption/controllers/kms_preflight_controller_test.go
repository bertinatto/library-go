...
// TestComputeEncryptionConfigSecret exercises the preflight thin wrapper around
// ComputeDesiredEncryptionConfig (planned key + endpoint override / rewrite).
func TestComputeEncryptionConfigSecret(t *testing.T) {
	...
	t.Run("no new key needed still returns existing write-key config for preflight", func(t *testing.T) {
		...
		// says needed=false. Preflight must reuse key 3 and rewrite its endpoint.
		ks := state.KeyState{
		...
		if kmsConfigs[0].Endpoint != preflightKMSSocketEndpoint {
			t.Fatalf("expected write-key endpoint rewritten to %s, got %s", preflightKMSSocketEndpoint, kmsConfigs[0].Endpoint)
		}
