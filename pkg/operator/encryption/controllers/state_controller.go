...
func (c *stateController) generateAndApplyCurrentEncryptionConfigSecret(ctx context.Context, queue workqueue.RateLimitingInterface, recorder events.Recorder, encryptedGRs []schema.GroupResource) error {
	result, err := ComputeDesiredEncryptionConfig(
		ctx,
		c.instanceName,
		nil, // unsupported config not needed when IncludePlannedKey is false
		encryptedGRs,
		c.deployer,
		c.secretClient,
		nil,
	...
	_, changed, applyErr := resourceapply.ApplySecret(ctx, c.secretClient, recorder, result.EncryptionSecret)
	if applyErr != nil {
		return applyErr
	}
