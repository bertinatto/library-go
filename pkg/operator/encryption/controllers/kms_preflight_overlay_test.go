...
func TestDryRunSecretsOverlay_CreateThenGet(t *testing.T) {
	...
	overlay := newDryRunSecretsGetter(fakeClient.CoreV1())
...
func TestDryRunSecretsOverlay_RecordedIn(t *testing.T) {
	live := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: "live-only", Namespace: "ns"},
	}
	fakeClient := fake.NewSimpleClientset(live)
	...
	require.Empty(t, overlay.RecordedIn("ns"), "live-only reads must not appear in recorded writes")
	require.Empty(t, overlay.RecordedIn("other-ns"))
	...
	recorded := overlay.RecordedIn("ns")
	require.Len(t, recorded, 2)
	names := map[string]*corev1.Secret{}
	for _, s := range recorded {
		names[s.Name] = s
	}
	require.Contains(t, names, "written-a")
	require.Contains(t, names, "written-b")
	require.Equal(t, []byte("v"), names["written-b"].Data["k"])

	names["written-a"].Data = map[string][]byte{"mutated": []byte("x")}
	reread := overlay.RecordedIn("ns")
	for _, s := range reread {
		if s.Name == "written-a" {
			require.NotEqual(t, []byte("x"), s.Data["mutated"])
		}
	}
}
