...
func newDryRunSecretsGetter(real corev1client.SecretsGetter) *dryRunSecretsGetter {
...
// RecordedIn returns deep copies of all secrets recorded by Create/Update in namespace.
// Live secrets that were only read are not returned. Returns an empty slice if no
// secrets were recorded in the namespace.
func (g *dryRunSecretsGetter) RecordedIn(namespace string) []*corev1.Secret {
	byName := g.written[namespace]
