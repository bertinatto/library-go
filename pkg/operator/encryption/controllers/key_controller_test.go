			...
			validateFunc: func(ts *testing.T, actions []clientgotesting.Action, targetNamespace string, targetGRs []schema.GroupResource) {
				...
				for _, action := range actions {
					if action.Matches("create", "secrets") {
						...
						expectedProviderConfig := `{"vault":{"image":"quay.io/rhn_support_rgangwar/mock-kms-plugin-vault:latest","vaultAddress":"vault-address","vaultNamespace":"vault-namespace","transitKey":"transit-key","transitMount":"transit-mount"}}`
